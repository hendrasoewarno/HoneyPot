...
Honeypot FTPS ini tidak disertai pembatasan jumlah thread, sehingga perlu dilakukan
pembatasan pada level firewall.
/sbin/iptables  -A INPUT -p tcp --syn --dport 2121 -m connlimit --connlimit-above 50 -j REJECT
FtpsSvr mensimulasikan server ftp untuk sebagai honeypot yang menarik penyerang
...
nohup python /path/to/ftpssvr.py &
'''
#!/usr/bin/env python3
import time
import datetime  
...
handler = TimedRotatingFileHandler('HPOTftps.log',
    when='midnight',
    interval=1,
    backupCount=30)
